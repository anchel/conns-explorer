...
import { describeTopicConfigs } from "@/utils/ipc.ts";
...
onMounted(() => {
  console.log('topic configs onMounted');
  fetchList();
})
...
const fetchList = async () => {
  ...
  try {
    const res = await describeTopicConfigs(route.params.cid as string, topic.name);
    console.log('fetch topic configs', res);
    if (res.resources && res.resources.length > 0) {
      const findResource = res.resources.find((r: any) => r.resourceType === 2 && r.resourceName === topic.name);
      if (findResource) {
    ...
    console.log('fetch topic configs error', e);
    ElMessage.error('Failed to fetch topic configs, please try again later');
  } finally {
